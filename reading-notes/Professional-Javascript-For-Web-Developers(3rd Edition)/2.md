...
+ 怪癖检测:怪癖实际上是浏览器实现中的存在的bug,例如早期的webkit中就存在一个怪癖,即它会在for-in循环中返回隐藏的属性.怪癖检测通常涉及到运行一小段代码,然后确定浏览器是否存在某个怪癖.由于怪癖检测与能力检测相比较效率更低,因此应该只在某个怪癖会干扰脚本运行的情况下使用.怪癖检测无法精确地检测特定浏览器和版本.
